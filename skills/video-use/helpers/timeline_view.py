...
def main() -> None:
    ...
    ap.add_argument("--n-frames", type=int, default=10, help="Frames in filmstrip (default 10)")
    ap.add_argument(
        "--transcript",
        type=Path,
        default=None,
        help="Path to transcript.json. Auto-resolves to <video_parent>/edit/transcripts/<stem>.json if omitted.",
    )
    ap.add_argument(
        "--edl",
        type=Path,
        default=None,
        help="(Not yet implemented) Full-project timeline from an EDL.",
    )
    args = ap.parse_args()
